...
class Form211Handler(FormHandler):
    ...
    def process(self, period_or_link: str) -> bool:
        ...
        with AnimatedWaitContext('Preparing tax form', self.config.is_verbose, self.config.is_debug):
        ...
        with AnimatedWaitContext('Retrieving tax form', self.config.is_verbose, self.config.is_debug):
            permit_data = {
                'ruc': self.cedula,
                'dv': self.dv,
                'periodo': period,
        ...
        with AnimatedWaitContext('Please wait! Processing tax form data', self.config.is_verbose, self.config.is_debug):
            for inp in inputs:
                name = inp.get('name', '')
                value = inp.get('value', '')
        ...
        # Submit the form
        with AnimatedWaitContext('Sending tax form', self.config.is_verbose, self.config.is_debug):
