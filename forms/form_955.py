...
class Form955Handler(FormHandler):
    ...
    def process(self, period_or_link: str) -> bool:
        ...
        with AnimatedWaitContext('Preparing receipt management', self.config.is_verbose, self.config.is_debug, self.config.mockup_mode):
        ...
        # Get operations list
        with AnimatedWaitContext('Getting access to receipt forms', self.config.is_verbose, self.config.is_debug, self.config.mockup_mode):
            operations_data = {'ruc': self.cedula}
            token_operations = self.encrypt_token(operations_data)

        with AnimatedWaitContext('Fetching receipt operations', self.config.is_verbose, self.config.is_debug, self.config.mockup_mode):
        ...
        with AnimatedWaitContext('Retrieving receipt forms', self.config.is_verbose, self.config.is_debug, self.config.mockup_mode):
        ...
        # Submit the talon
        with AnimatedWaitContext(f'Sending tax form {self.FORM}', self.config.is_verbose, self.config.is_debug, self.config.mockup_mode):
            talon_data = {
                'periodo': int(period),
        ...
        with AnimatedWaitContext('Submitting receipt form', self.config.is_verbose, self.config.is_debug, self.config.mockup_mode):
